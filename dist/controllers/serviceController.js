...
class ServiceController {
    constructor() {
        ...
        this.executeStart = (serviceName) => tslib_1.__awaiter(this, void 0, void 0, function* () {
            if (!serviceName) {
                for (const service of this.allServices) {
                    try {
                        ...
                        (0, console_1.error)(`Failed to start ${service.service}: ${e.message}`);
                    }
                }
                (0, console_1.success)('Successfully started all Jale services.');
                return true;
            }
            for (const service of this.allServices) {
                if (service.service.includes(serviceName)) {
                    try {
                        ...
                        (0, console_1.success)(`Successfully started ${serviceName}.`);
                        return true;
                    }
                    catch (e) {
                        (0, console_1.error)(`Failed to start ${service.service}: ${e.message}`);
            ...
            (0, console_1.warning)(`Invalid service: ${serviceName}`);
            return false;
        });
        this.executeStop = (serviceName) => tslib_1.__awaiter(this, void 0, void 0, function* () {
            if (!serviceName) {
                for (const service of this.allServices) {
                    try {
                        ...
                        (0, console_1.error)(`Failed to stop ${service.service}: ${e.message}`);
                    }
                }
                (0, console_1.success)('Successfully stopped all Jale services.');
                return true;
            }
            for (const service of this.allServices) {
                if (service.service.includes(serviceName)) {
                    try {
                        ...
                        (0, console_1.success)(`Successfully stopped ${serviceName}`);
                        return true;
                    }
                    catch (e) {
                        (0, console_1.error)(`Failed to stop ${service.service}: ${e.message}`);
            ...
            (0, console_1.error)(`Invalid service: ${serviceName}`);
            return false;
        });
            ...
            if (!serviceName) {
                for (const service of this.allServices) {
                    try {
                        ...
                        (0, console_1.error)(`Failed to restarted ${service.service}: ${e.message}`);
                    }
                }
                (0, console_1.success)('Successfully restarted all Jale services.');
                return true;
            }
            for (const service of this.allServices) {
                if (service.service.includes(serviceName)) {
                    try {
                        ...
                        (0, console_1.success)(`Successfully restarted ${serviceName}.`);
                        return true;
                    }
                    catch (e) {
                        (0, console_1.error)(`Failed to restarted ${service.service}: ${e.message}`);
        ...
        this.controlService = (service, action) => tslib_1.__awaiter(this, void 0, void 0, function* () {
            if (service instanceof mysql_1.default) {
                const linkedDatabase = yield (0, database_1.getLinkedDatabase)();
                if (linkedDatabase.service !== service.service) {
                    return false;
                }
            }
            if (service instanceof phpFpm_1.default) {
                const linkedPhpVersion = yield (0, phpFpm_2.getLinkedPhpVersion)();
            ...
            switch (action) {
                case 'start':
                    (0, console_1.info)(`Starting ${service.service}...`);
                    yield service.start();
                    break;
                case 'stop':
                    (0, console_1.info)(`Stopping ${service.service}...`);
                    yield service.stop();
                    break;
                case 'restart':
                    (0, console_1.info)(`Restarting ${service.service}...`);
